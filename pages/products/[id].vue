            ...
            <div class="thumbs-viewport">
              <div class="thumbs-list">
                ...
                >
                  <img
                    :src="img"
                    :alt="`${product.title}-${thumbStartIndex + index}`"
                  />
                </button>
              </div>
            </div>
        ...
        <section class="product-info">
          ...
          <div v-if="product.upsells?.length" class="upsell-section">
            <h3 class="upsell-title">
              <ZapIcon class="icon-zap" /> {{ copy.addonTitle }}
            </h3>
            <label
            ...
            >
              ...
              <div class="item-price">
                ...
                <span v-if="item.compareAtPrice" class="old-price">{{
          ...
          <div class="accordion-group">
            ...
            >
              ...
              >
                <div class="specs-table">
                  <div
                    v-for="spec in product.specTable || []"
            ...
            >
              ...
              >
                <ul class="box-list">
                  <li v-for="item in product.boxItems || []" :key="item">
                    {{ item }}
                  </li>
                </ul>
              </div>
            ...
            <div
              class="accordion-item"
              :class="{ 'is-open': activeAccordion === 'faq' }"
            >
              <button
                type="button"
                class="accordion-header"
...
const visibleThumbImages = computed(() =>
  galleryImages.value.slice(
    thumbStartIndex.value,
    thumbStartIndex.value + THUMB_WINDOW,
  ),
);
...
const resolvedSpecs = computed(() =>
  (Array.isArray(product.value?.specs) ? product.value.specs : [])
    .slice(0, 6)
    .map((spec: any) => ({
      ...spec,
      icon:
...
const attributeKeys = computed(() =>
  sortAttributeKeys(Object.keys(attributeOptions.value || {})),
);
const selectedSku = computed(() =>
  findSelectedSku(product.value?.skuList || [], selection),
);
const selectedSkuAvailable = computed(() =>
  Boolean(selectedSku.value && isSkuAvailable(selectedSku.value)),
);
const selectedPrice = computed(
  () => selectedSku.value?.price || product.value?.price || 0,
);
const selectedCompareAtPrice = computed(
  () => selectedSku.value?.compareAtPrice || product.value?.compareAtPrice || 0,
);
...
const toggleAccordion = (
  panelName: "quick-know" | "specification" | "in-the-box" | "faq",
) => {
...
const optionThumbnail = (attributeKey: string, option: string) => {
  const skuList = Array.isArray(product.value?.skuList)
    ? product.value.skuList
    : [];
  const matched = skuList.find(
    (sku: any) => sku.attributes?.[attributeKey] === option,
  );
  return (
    mergeProductImages(
      matched?.images,
...
const cycleImage = (step: number) => {
  ...
  const nextIndex =
    (currentIndex + step + galleryImages.value.length) %
...
.thumb-stepper:hover:not(:disabled) {
  border-color: #d1d5db;
  color: #111;
}
